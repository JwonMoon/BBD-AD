...
# 5. 그래프 그리기
plt.figure(figsize=(12, 14))  # 그래프 크기 조정 (세로 길이 증가)
...
# CPU Usage
plt.subplot(4, 2, 6)
plt.plot(time, data['cpu_usage [%]'], label='CPU Usage (%)', color='cyan')
plt.title('CPU Usage')
plt.xlabel('Time')
plt.ylabel('Usage (%)')
plt.grid(True)
...
# System Memory Used
plt.subplot(4, 2, 7)
plt.plot(time, data['memory_used [MiB]'], label='Memory Used (MiB)', color='magenta')
plt.title('System Memory Used')
plt.xlabel('Time')
plt.ylabel('Memory (MiB)')
plt.grid(True)
plt.legend()
...
plt.subplot(4, 2, 8)
plt.plot(time, data['memory_utilization [%]'], label='Memory Utilization (%)', color='brown')
