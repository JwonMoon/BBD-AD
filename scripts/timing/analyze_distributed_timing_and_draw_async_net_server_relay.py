...
def draw_block(ax, start, duration, level, label, color, threshold=7.0):
    ax.add_patch(patches.Rectangle((start, level - 0.2), duration, 0.4, color=color, alpha=0.6))
    if not label:
        return
    if duration < threshold:
        x_text = start + duration + max(1.5*duration , 10)
        y_text = level + min(0.22*duration , 0.5)
        ax.annotate(
            f"{label}\n{duration:.2f}",
...
def draw_diagram_from_mean_durations(mean):
    ...
    def compute_frame(prev_end):
        ...
        f["T_bb_start"] = prev_end
        f["T_bb_rx_start"] = f["T_bb_start"]
        f["T_bb_pp_start"] = f["T_bb_rx_start"] + mean["D_bb_rx"]
        f["T_bb_net_start"] = f["T_bb_pp_start"] + mean["D_bb_pp"]
        f["T_bb_bb_gen_start"] = f["T_bb_net_start"] + mean["D_bb_net"]
        f["T_bb_bb_pub_start"] = f["T_bb_bb_gen_start"] + mean["D_bb_bb_gen"]
        f["T_bb_bb_pub_end"] = f["T_bb_bb_pub_start"] + mean["D_bb_bb_pub"]
        f["T_bb_log_start"] = f["T_bb_net_start"] + mean["D_bb_net"] + mean["D_bb_net_to_log"]

        f["T_br_start"] = f["T_bb_bb_pub_start"] + mean["D_transfer_delay"]
        f["T_br_rx_start"] = f["T_br_start"]

        f["T_br_recv_start"] = f["T_br_rx_start"] + mean["D_br_from_bytes"]
        f["T_br_restore_start"] = f["T_br_recv_start"] + mean["D_br_recv"]
        f["T_br_net_start"] = f["T_br_restore_start"] + mean["D_br_restore"]


        f["T_br_pid_start"] = f["T_br_net_start"] + mean["D_br_net"]
        f["T_br_pid_end"] = f["T_br_pid_start"] + mean["D_br_pid"]
        f["T_br_pub_start"] = f["T_br_pid_end"] + mean["D_br_pid_to_pub"]
        f["T_br_pub_end"] = f["T_br_pub_start"] + mean["D_br_pub"]

        f["T_rl_pub_start"] = f["T_br_pub_start"] + mean["D_control_delay"]
    ...
    # 프레임 0
    f0 = compute_frame(0)
    # 프레임 1: backbone 종료 이후 D_tick 반영
    f1_start = f0["T_bb_start"] + mean["D_tick"]
    ...
    for f in [f0, f1]:
        ...
        draw_block(ax, f["T_bb_pp_start"], mean["D_bb_pp"], y_levels["Backbone"], "pp", cobalt_blue, 4)
        draw_block(ax, f["T_bb_net_start"], mean["D_bb_net"], y_levels["Backbone"], "net", cobalt_blue)
        draw_block(ax, f["T_bb_bb_gen_start"], mean["D_bb_bb_gen"], y_levels["Backbone"], "tx", cobalt_blue)
    ...
    ax.set_xlim(-10, f1["T_br_pub_end"] + 20)
    ax.set_ylim(0.4, 3.9)
...
def main(args):
    ...
    relay_path    = f"{base}_relay/relay_timing.csv"
    ...
    df_rl = pd.read_csv(relay_path)

    # 🔹 Step 기준 병합
    merged = pd.merge(df_bb, df_br, on="step", suffixes=("_bb", "_br"))
    merged = pd.merge(merged, df_rl, on="step", suffixes=("", "_rl"))

    # 🔹 (N+1) 비교용
    merged["T_proc_start_next"] = merged["T_proc_start"].shift(-1)
    merged["T_tx_start_next"] = merged["T_tx_start"].shift(-1)
    merged["T_rl_pub_start_next"] = merged["T_rl_pub_start"].shift(-1)
    ...
    merged["D_bb_bb_gen"] = (merged["T_bb_pub_start"] - merged["T_tx_bb_start"]) * 1000 # include tx time
    merged["D_bb_bb_pub"] = (merged["T_bb_pub_end"] - merged["T_bb_pub_start"]) * 1000 # include tx time
    merged["D_bb_net_to_log"] = (merged["T_log_start"] - merged["T_bb_end"]) * 1000
    merged["D_bb_log"] = (merged["T_log_end"] - merged["T_log_start"]) * 1000
    
    merged["D_transfer_delay"] = (merged["T_cb_start"] - merged["T_bb_pub_start"]) * 1000

    merged["D_br_total"] = (merged["T_tx_end"] - merged["T_cb_start"]) * 1000
    merged["D_br_rx"] = (merged["T_rx_end"] - merged["T_cb_start"]) * 1000
    
    merged["D_br_from_bytes"] = (merged["T_recv_start"] - merged["T_rx_start"]) * 1000
    merged["D_br_recv"] = (merged["T_recv_end"] - merged["T_recv_start"]) * 1000
    # merged["D_br_json_load"] = (merged["T_restore_start"] - merged["T_recv_end"]) * 1000
    merged["D_br_restore"] = (merged["T_restore_end"] - merged["T_restore_start"]) * 1000

    merged["D_br_net"] = (merged["T_br_end"] - merged["T_rx_end"]) * 1000
    merged["D_br_pid"] = (merged["T_pid_end"] - merged["T_br_end"]) * 1000
    merged["D_br_pid_to_pub"] = (merged["T_tx_start"] - merged["T_pid_end"]) * 1000
    merged["D_br_pub"] = (merged["T_tx_end"] - merged["T_tx_start"]) * 1000

    merged["D_control_delay"] = (merged["T_rl_pub_start"] - merged["T_tx_start"]) * 1000

    merged["D_rl_pub"] = (merged["T_rl_pub_end"] - merged["T_rl_pub_start"]) * 1000

    merged["D_output"] = (merged["T_tx_start_next"] - merged["T_tx_start"]) * 1000
    merged["D_rl_output"] = (merged["T_rl_pub_start_next"] - merged["T_rl_pub_start"]) * 1000
    ...
    metric_list = [
        ...
        ("⑥-3 backbone net 실행 시간", "D_bb_net"),
        ("⑥-4 backbone feature tx 실행 시간", "D_bb_bb_gen"),
        ("⑥-4 backbone feature send 실행 시간", "D_bb_bb_pub"),
        
        ("⑦ transfer delay", "D_transfer_delay"),
        
        ("⑧ branch total 실행 시간", "D_br_total"),
        ("⑧-1 branch rx 실행 시간", "D_br_rx"),
        ("  --- 1) D_br_from_bytes", "D_br_from_bytes"),   
        ("  --- 2) D_br_recv", "D_br_recv"),      
        # ("  --- 3) D_br_json_load", "D_br_json_load"),
        ("  --- 3) D_br_restore", "D_br_restore"), 

        ("⑧-2 branch net 실행 시간", "D_br_net"),
        ("  --- 1) T_bb_perception", "T_bb_perception"),   
        ("  --- 2) T_bb_speed_branch", "T_bb_speed_branch"),      
        ("  --- 3) T_bb_measurements", "T_bb_measurements"),
        ("  --- 4) T_bb_join_traj", "T_bb_join_traj"),   
        ("  --- 5) T_bb_branch_traj", "T_bb_branch_traj"),
        ("  --- 6) T_bb_pred_wp", "T_bb_pred_wp"),      
        ("  --- 7) init_att", "T_br_init_att"),      
        ("  --- 8) join_ctrl", "T_br_join_ctrl"),      
        ("  --- 9) branch_ctrl", "T_br_branch_ctrl"),      
        ("  --- 10) action_head", "T_br_action_head"),      
        ("  --- 11) future_feature_action", "T_br_future_feature_action"),      
        ("⑧-3 branch pid 실행 시간", "D_br_pid"),
        ("⑧-4 branch tx 실행 시간", "D_br_pid_to_pub"),
        ("⑧-5 branch send  실행 시간", "D_br_pub"),
        
        ("⑩ D_control_delay", "D_control_delay"),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip_initial", type=int, default=10)
