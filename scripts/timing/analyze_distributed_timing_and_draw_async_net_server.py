...
def draw_diagram_from_mean_durations(mean):
    ...
    draw_interval(ax, f0["T_bb_start"], y_levels["Backbone"], f1["T_bb_start"], y_levels["Backbone"], "tick", mean["D_tick"], 1.0)
...
def main(args):
    ...
    merged["D_br_pub"] = (merged["T_tx_end"] - merged["T_pid_end"]) * 1000 # include

    merged["D_output"] = (merged["T_tx_start_next"] - merged["T_tx_start"]) * 1000
    # merged["D_control_delay"] = (merged["T_br_cb_start"] - merged["T_tx_start"]) * 1000
    ...
    metric_list = [
        ...
        ("⑧-4 branch tx & send  실행 시간", "D_br_pub"),
