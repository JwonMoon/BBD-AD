...
### planning ###
planning_steps = 6
use_col_optim = True #충돌 회피 최적화 적용.
...
### <UniAD 모델 구조>
model = dict(
    type="UniAD",   ### type="UniAD" → mmcv/models/detectors/uniad_e2e.py의 UniAD 클래스.
    ...
    ### image backbone :UniAD 모델의 내부 동작
    img_backbone=dict(
        type="ResNet",
        depth=101,
        num_stages=4,
    ...
    ### image neck :FPN(Feature Pyramid Network)으로 다중 스케일 특징 통합
    img_neck=dict(
        type="FPN",
    ...
    ### Head_1.객체 탐지 및 추적 :BEVFormerTrackHead로 이미지 특징을 BEV로 변환
    pts_bbox_head=dict(
        ...
        fut_steps=fut_steps,
        transformer=dict(
            type="UniADPerceptionTransformer", ### <-트랜스포머
    ...
    seg_head=dict(
        ...
        with_box_refine=True,
        transformer=dict(
            type='SegDeformableTransformer', ### <-트랜스포머
            encoder=dict(
    ...
    occ_head=dict(
        ...
        transformer_decoder=dict(
            type='DetrTransformerDecoder', ### <-트랜스포머
            return_intermediate=True,
            num_layers=5,
            transformerlayers=dict(
                type='DetrTransformerDecoderLayer',
    ...
    ### Head_4.궤적 예측 :MotionHead로 주변 객체와 자차의 미래 궤적 예측.
    motion_head=dict(
        type='MotionHead',
        bev_h=bev_h_,
        bev_w=bev_w_,
        ...
        transformerlayers=dict(
            type='MotionTransformerDecoder', ### <-트랜스포머
            pc_range=point_cloud_range,
            embed_dims=_dim_,
            num_layers=3,
            transformerlayers=dict(
    ...
    ### Head_5.경로 계획 :PlanningHeadSingleMode로 자차의 주행 궤적(sdc_traj) 생성
    planning_head=dict(
