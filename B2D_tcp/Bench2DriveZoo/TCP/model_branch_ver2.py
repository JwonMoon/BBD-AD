...
class TCPBranch(nn.Module):
	...
	def forward(self, j_traj, measurement_feature, cnn_feature, target_point):
		timing = {}
		outputs = {}
		t0 = time.time()
		...
		t7 = time.time()
		timing['future_feature_action'] = (t7- t6)*1000
	...
	def control_pid(self, waypoints, velocity, target):
		...
		delta = float(np.clip(desired_speed - speed, 0.0, self.config.clip_delta))
