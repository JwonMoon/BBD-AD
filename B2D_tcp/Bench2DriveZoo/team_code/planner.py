...
class RoutePlanner(object):
	...
	def set_route(self, global_plan, gps=False, global_plan_world=None):
		self.route.clear()
		print(f"[planner.py] Setting route with {len(global_plan)} waypoints")
		# print(f"[planner.py] global_plan: {global_plan}, global_plan_world: {global_plan_world}")

		if global_plan_world:
			for (pos, cmd), (pos_word, _) in zip(global_plan, global_plan_world):
				print(f"[planner.py] * Waypoint: pos={pos}, cmd={cmd}, world_pos={pos_word}")
				if gps:
					if not isinstance(pos, dict) or 'lat' not in pos or 'lon' not in pos:
						# self.logger.error(f"Invalid GPS data: {pos}")
						print(f"[planner.py] Invalid GPS data: {pos}")
						continue
					...
					if not self._is_valid_gps(pos_array):
						...
						print(f"[planner.py] Invalid GPS coordinates: {pos_array}")
						continue
					pos = self.gps_to_location(pos_array)

				else:
					if not isinstance(pos, dict) or 'location' not in pos:
						# self.logger.error(f"Invalid position data: {pos}")
						print(f"[planner.py] Invalid position data: {pos}")
						continue
					pos = np.array([pos['location']['x'], pos['location']['y']]) #jw
				self.route.append((pos, cmd, pos_word))
		else:
			for pos, cmd in global_plan:
				# print(f"[planner.py] Waypoint: pos={pos}, cmd={cmd}")
				if gps:
					if not isinstance(pos, dict) or 'lat' not in pos or 'lon' not in pos:
						# self.logger.error(f"Invalid GPS data: {pos}")
						print(f"[planner.py] Invalid GPS data: {pos}")
					...
					if not isinstance(pos, dict) or 'location' not in pos:
						# self.logger.error(f"Invalid position data: {pos}")
						print(f"[planner.py] Invalid location data: {pos}")
						continue
					pos = np.array([pos['location']['x'], pos['location']['y']])
				self.route.append((pos, cmd))
		# self.logger.info(f"planner.set_route() finished")
		print(f"[planner.py] planner.set_route() finished")
