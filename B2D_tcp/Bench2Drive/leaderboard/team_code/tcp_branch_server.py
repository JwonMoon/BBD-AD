...
# def restore_tensor(msg, config): # 1)msg gen 병목
#     cnn = torch.tensor(msg['cnn_feature'], dtype=torch.float32).view(1, 512, 8, 29).cuda()
#     meas = torch.tensor(msg['measurement_feature'], dtype=torch.float32).view(1, 128).cuda()
#     traj = torch.tensor(msg['traj_hidden_state'], dtype=torch.float32).view(1, config.pred_len, 256).cuda()
#     gt_velocity = torch.tensor([[msg['gt_velocity']]], dtype=torch.float32).cuda()
#     target = torch.tensor([msg['target_point']], dtype=torch.float32).cuda()
#     command = torch.tensor(msg['command'], dtype=torch.float32).view(1, 6).cuda()
#     pred_wp = torch.tensor(msg['pred_wp'], dtype=torch.float32).view(1, config.pred_len, 2).cuda()
#     state = torch.cat([gt_velocity / 12.0, target, command], dim=1)
#     return cnn, meas, traj, gt_velocity, target, command, pred_wp, state

def restore_tensor(msg, config):
    cnn = torch.from_numpy(
        np.frombuffer(msg['cnn_feature'], dtype=np.float32).reshape(msg['cnn_shape']).copy()
    ).cuda()

    meas = torch.from_numpy(
        np.frombuffer(msg['measurement_feature'], dtype=np.float32).reshape(msg['measurement_shape']).copy()
    ).cuda()

    traj = torch.from_numpy(
        np.frombuffer(msg['traj_hidden_state'], dtype=np.float32).reshape(msg['traj_shape']).copy()
    ).cuda()

    gt_velocity = torch.tensor([[msg['gt_velocity']]], dtype=torch.float32).cuda()  # shape (1, 1)

    # 여기가 의심됨 — shape이 (1, 2)가 되도록 보장
    target = torch.tensor(msg['target_point'], dtype=torch.float32).view(1, 2).cuda()
    ...
    # state는 (1, 9) 형태여야 함
    state = torch.cat([gt_velocity / 12.0, target, command], dim=1)  # (1, 1+2+6) = (1, 9)
...
def start_branch_server(ckpt_path, debug_mode, orin1_ip='192.168.20.1', port_recv=9999, port_send=9998):
    ...
    while True:
        try:
            ...
            step = msg['step']
            cnn, meas, traj, gt_vel, target, cmd, pred_wp, state = restore_tensor(msg, config)
            T_restore_end = time.time()
            T_rx_end = time.time()

            # 2. Inference
            T_br_start = time.time()
            pred = model(cnn, meas, traj)
            T_br_end = time.time()

            # 3. PID
            T_pid_start = time.time()
            steer_ctrl, throttle_ctrl, brake_ctrl, meta_ctrl = model.process_action(pred, int(torch.argmax(cmd)), gt_vel, target)
            steer_traj, throttle_traj, brake_traj, meta_traj = model.control_pid(pred_wp, gt_vel, target)
            T_pid_end = time.time()
