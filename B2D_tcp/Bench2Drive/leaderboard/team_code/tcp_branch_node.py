...
class TCPBranchNode(Node):
    def __init__(self, ckpt_path, save_path, debug_mode):
        ...
        # self.publisher = self.create_publisher(TCPBackboneOutput, '/tcp/backbone_output', QoSProfile(depth=1))
        self.control_pub = self.create_publisher(CarlaEgoVehicleControl, '/tcp/vehicle_control_cmd', QoSProfile(depth=1))
    ...
    # def process_step(self):
        # self.get_logger().warning(f"[Branch] Process step called, step={self.step}")
        ...
        # self.pid_metadata = {
        #     'step': self.step,
        #     'cnn_feature': msg.cnn_feature[:10],
        #     'measurement_feature': msg.measurement_feature,
        #     'traj_hidden_state': msg.traj_hidden_state[:10],
    ...
    def backbone_callback(self, msg):
        ...
        # target_point = [float(x) for x in target_point.squeeze().tolist()]
        ...
        # Inference
        self.get_logger().warning(f"- process_step(): branch net()")
        t3 = time.perf_counter()
        # break net!
        ...
        self.pid_metadata = {
            ...
            'ros_time_ns': ros_time_ns,
            'branch_inference_ms': (t4 - t3) * 1000,
...
def main():
    ...
    rclpy.init()
    node = TCPBranchNode(args.ckpt_path, args.save_path, args.debug)
    try:
        rclpy.spin(node)
